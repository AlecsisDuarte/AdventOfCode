...
the banks. To do this, it removes all of the blocks from the selected bank, then moves to the next 
(by index) memory bank and inserts one of the blocks. It continues doing this until it runs out 
...
banks get two blocks each, and the third bank gets one back. The final result looks like 
this: 2 4 1 2.
...
class Node{
    ...
    public Node(int key, int maxIndex){
        this.key = key;
...
public class MemoryReallocation{
    ...
    public static int[] redistributionCycles(int[] banks){
        int[] tmpBank = banks.clone();
        int maxIndex = getKeyAndMax(tmpBank);
        int distribute = tmpBank[maxIndex];
        tmpBank[maxIndex] = 0;
        while(distribute-- > 0){
            if(++maxIndex == tmpBank.length) maxIndex = 0;
            ++tmpBank[maxIndex];
        }
        return tmpBank;
    }

    public static int getKeyAndMax(int[] banks){
        int maxIndex = 0;
        for(int i = 0; i < banks.length; i++){
            maxIndex = banks[i] > banks[maxIndex] ? i : maxIndex;
        }
        return maxIndex;
    }

    public static void printArray(int[] banks){
        System.out.print("\nArray = ");
        for(int n : banks) System.out.print(n + ", ");
        System.out.println();
    }

    public static boolean equalArrays(int[] f, int[] s){
    ...
    public static void main(String[] args){
        try{
            ...
            int index = 0;
            while(in.hasNextInt()) banks[index++] = in.nextInt();
            int[] slow = redistributionCycles(banks);
            int[] fast = redistributionCycles(redistributionCycles(banks));
            while (!equalArrays(slow, fast)){
                slow = redistributionCycles(slow);
                fast = redistributionCycles(redistributionCycles(fast));
            }
            int pos = 0;
            slow = banks;
            while (!equalArrays(slow, fast)){
                slow = redistributionCycles(slow);
                fast = redistributionCycles(fast);
                pos++;
            }

            fast = redistributionCycles(slow);
            int count = 1;
            while(!equalArrays(slow, fast)){
                fast = redistributionCycles(fast);
                ++count;
            ...
            in.close();
            System.out.println("Part 1 Cycles = " + (pos + count));
            System.out.println("Part 1 Loop Size = " + (count));
