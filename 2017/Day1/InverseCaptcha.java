...
public class InverseCaptcha{
    public static void main(String[] args){
        ...
        for(int i = 0; i < s.length(); i++){
            char c = s.charAt(i);
            if(c == s.charAt( i == s.length() -1 ? 0 : i+1)){
                count += c - 48;
            }
            int secondIndex = (s.length() /2) + i;
            while(secondIndex > s.length() - 1) secondIndex -= s.length();
            if(c == s.charAt(secondIndex)){
                count2 += c -48;
            }
        }
        System.out.println("For Part1: "+ count);
        System.out.println("For Part2: "+ count2);
    }
}