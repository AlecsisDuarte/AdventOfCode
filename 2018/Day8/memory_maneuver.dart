/**
  ...
  kind of navigation system! Activating the navigation system produces more bad news: 
  ...
  and it contains all other nodes in the tree (or contains nodes that contain nodes, 
  and so on).
  ...
  to those child nodes. A metadata entry of 1 refers to the first child node, 2 to the second, 
...
void main() async {
  File input = InputReader.openInputFile('Day8', 'input.txt');
  if (!await input.exists()) {
    print('Input file not found');
    return;
  }

  input.readAsString().then((line) {
    List<int> data = getLicenseData(line);

    getMetadataSum(data).then(
        (metadataSum) => print('Metadata Sum: ${metadataSum.metadataSum}'));

    getRootValue(data)
        .then((rootValue) => print('Root value: ${rootValue.metadataSum}'));
  });
}

/** Cast the string line into an Int List */
List<int> getLicenseData(String line) {
  return line.split(' ').map<int>((n) => int.parse(n)).toList();
}
...
class LicenseMetadata {
  ...
  int metadataSum;
  int rootNodeValue;
  LicenseMetadata(this.index, this.metadataSum);
}

Future<LicenseMetadata> getMetadataSum(List<int> data, {int index = 0}) async {
  int metadataSum = 0;
  int nodes = 0;
  ...
  do {
    ...
    while (nodes-- > 0) {
      var sonLicenseMetadata = await getMetadataSum(data, index: ++index);
      index = sonLicenseMetadata.index;
      metadataSum += sonLicenseMetadata.metadataSum;
    }
    while (entries-- > 0) {
      metadataSum += data[++index];
    }
  } while (index < data.length && nodes > 0);

  return LicenseMetadata(index, metadataSum);
}

Future<LicenseMetadata> getRootValue(List<int> data, {int index = 0}) async {
  int metadataSum = 0;
  int nodes = 0;

  do {
    List<int> childValues = [];
    nodes = data[index];
    int entries = data[++index];
    while (nodes-- > 0) {
      var sonLicenseMetadata = await getRootValue(data, index: ++index);
      index = sonLicenseMetadata.index;
      childValues.add(sonLicenseMetadata.metadataSum);
    }

    if (childValues.length > 0) {
      while (entries-- > 0) {
        int childIndex = data[++index] - 1;
        if (childIndex >= 0 && childValues.length > childIndex) {
          metadataSum += childValues[childIndex];
        }
      }
    } else {
      while (entries-- > 0) {
